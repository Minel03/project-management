...
interface KanbanColumnProps {
  ...
  tasks: any[];
  onDragOver: (e: React.DragEvent<HTMLDivElement>, status: 'Todo' | 'In Progress' | 'Done') => void;
  onDragLeave: () => void;
  onDrop: (e: React.DragEvent<HTMLDivElement>, status: 'Todo' | 'In Progress' | 'Done') => void;
...
export function KanbanColumn({
  ...
  return (
    <div
      ...
      className={cn(
        'kanban-dropzone p-4 rounded-2xl bg-slate-900/20 border border-slate-900 min-h-[400px] flex flex-col',
      )}
    >
      ...
      <div className="flex-1 space-y-3 overflow-y-auto">
        {tasks.map(task => (
          ...
          >
            ...
            <h4 className="text-xs font-bold text-slate-200 mb-1 pr-6 truncate" style={status === 'Done' ? { textDecoration: 'line-through', color: '#6b7280' } : {}}>{task.title}</h4>
            <p className="text-[11px] text-slate-400 line-clamp-2 mb-3">{task.description || 'No description.'}</p>
            <div className="flex items-center justify-between text-[9px] text-slate-600">
              {task.assignee_name ? (
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded-full bg-indigo-950 border border-indigo-500/20 flex items-center justify-center uppercase text-[8px] font-bold text-indigo-300" title={`Assigned to ${task.assignee_name}`}>
